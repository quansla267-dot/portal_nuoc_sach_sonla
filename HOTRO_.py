    ...
    def thuc_hien(self):
        ...
        if kq_commit.returncode != 0:
            if "nothing to commit" in kq_commit.stdout + kq_commit.stderr:
                ...
                return
            self.ghi(f"❌ Lỗi: {kq_commit.stderr}")
            return
        ...
        if kq_push.returncode == 0:
            self.ghi("")
            self.ghi("🎉 THÀNH CÔNG! app.py đã được tạo & đẩy lên GitHub ✅")
            self.ghi("→ Streamlit cập nhật sau 1-2 phút")
            messagebox.showinfo("Hoàn thành", "✅ app.py mới đã được đẩy lên GitHub!\nStreamlit sẽ tự cập nhật.")
        else:
            self.ghi(f"❌ Lỗi đẩy lên GitHub: {kq_push.stderr}")
            messagebox.showerror("Lỗi", "Không đẩy được lên GitHub!\nKiểm tra mạng hoặc đăng nhập")
