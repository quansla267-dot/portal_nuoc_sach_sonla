...
'''.replace("{TEN_HE_THONG}", self.ten_he_thong)
    ...
    def doc_ten_tu_file_mau(self, duong_dan_file_mau=""):
        """Đọc tên hệ thống từ ô A1 file mẫu — ƯU TIÊN lấy từ file vừa chọn"""
        # 1. Đọc từ file mẫu người dùng vừa chọn
        if duong_dan_file_mau and os.path.isfile(duong_dan_file_mau):
            try:
                df = pd.read_excel(duong_dan_file_mau, header=None)
                ...
                self.ghi(f"⚠️ Không đọc được ô A1: {str(e)}")
        
        # 2. Nếu không có → tìm trong thư mục chương trình
        for ten_file in os.listdir(self.thu_muc_goc):
            if ten_file.lower().endswith((".xlsx", ".xls")):
                try:
                    ...
                    if ten_moi:
                        self.ten_he_thong = ten_moi
                        self.ghi(f"📌 Tên hệ thống (tìm tự động): {ten_moi}")
                        return True
                except:
                    pass
        return False
    ...
    def tao_giao_dien(self):
        ...
        self.ghi_chu = tk.StringVar(value=f"Cập nhật: {datetime.now().strftime('%d/%m/%Y %H:%M')}")
    ...
    def chon_file_mau(self):
        ...
        if f:
            ...
            # Đọc tên hệ thống ngay khi chọn file
            self.doc_ten_tu_file_mau(f)
    ...
    def doc_danh_sach_tu_excel(self, duong_dan):
        try:
            ...
            anh_xa = {
                ...
                "MatKhau": "MatKhau", "Mật khẩu": "MatKhau",
                "VaiTro": "VaiTro", "Khoa": "VaiTro", "Vai trò": "VaiTro",
                "DonVi": "DonVi", "Tên Xã/Phường": "DonVi", "Đơn vị": "DonVi"
            }
    ...
    def thuc_hien(self):
        ...
        if not self.duong_dan_excel_tk.get().strip():
            messagebox.showwarning("Thiếu file", "Chọn file tài khoản!")
            return
        if not self.kiem_tra_git():
            messagebox.showerror("Thiếu Git", "Cài Git trước nhé!")
            return

        # Đọc tên hệ thống LẦN CUỐI trước khi tạo app.py
        self.doc_ten_tu_file_mau(self.duong_dan_file_mau.get().strip())

        # Đọc tài khoản
        self.ghi("📖 Đọc danh sách tài khoản...")
        ds = self.doc_danh_sach_tu_excel(self.duong_dan_excel_tk.get().strip())
        if ds is None: return
        self.ghi(f"✅ Đọc được {ds.count('TaiKhoan')} tài khoản")

        # Tạo app.py với tên mới
        self.ghi("⚙️ Tạo app.py...")
        self.tao_app_py_moi(ds)
        self.ghi("✅ Đã cập nhật app.py")

        # Sao chép file mẫu
        duong_dan_mau = self.duong_dan_file_mau.get().strip()
        if duong_dan_mau and os.path.isfile(duong_dan_mau):
            try:
                ...
                # Sao chép mới
                shutil.copy2(duong_dan_mau, os.path.join(self.thu_muc_mau, ten_file))
                self.ghi(f"✅ File mẫu: {ten_file}")
            except Exception as e:
                self.ghi(f"❌ Lỗi sao chép: {str(e)}")

        # Git & Push
        if not os.path.exists(".git"):
            self.ghi("🔧 Khởi tạo Git...")
        ...
        if kq_commit.returncode != 0:
            if "nothing to commit" not in kq_commit.stdout + kq_commit.stderr:
                self.ghi(f"❌ Lỗi Commit: {kq_commit.stderr}")
                return
            self.ghi("ℹ️ Không có thay đổi mới")
        else:
            self.ghi("📤 Đẩy lên GitHub...")
            ...
            if kq_push.returncode == 0:
                self.ghi("🎉 THÀNH CÔNG! ✅")
                messagebox.showinfo("Hoàn thành", "✅ Tải lại Streamlit nhé!")
            else:
                self.ghi(f"❌ {kq_push.stderr}")
